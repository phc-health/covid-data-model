...
def get_run_artifact_path(fips, artifact, output_dir=None) -> str:
    ...
    if artifact is RunArtifact.RT_INFERENCE_REPORT:
        ...
        path = os.path.join(
            output_dir, "forecast", f"Forecast_var_unscaled_{state_obj.name}__{fips}.pdf",
        )
    elif artifact is RunArtifact.MODEL:
        path = os.path.join(output_dir, "forecast", "model.h5")

    elif artifact is RunArtifact.FORECAST_LOSS:
        path = os.path.join(output_dir, "forecast", "Forecast_loss.pdf",)

    elif artifact is RunArtifact.FORECAST_LOSS_MAPE:
        path = os.path.join(output_dir, "forecast", "Forecast_loss_mape.pdf",)

    elif artifact is RunArtifact.FORECAST_VAR_SCALED:
        path = os.path.join(
            output_dir, "forecast", f"Forecast_var_scaled_{state_obj.name}__{fips}.pdf",
        )
    ...
    elif artifact is RunArtifact.PERCENTILE_PLOT:
        path = os.path.join(output_dir, "forecast", "percentile",)
