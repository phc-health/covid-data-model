...
class NYTimesFields(GetByValueMixin, ValueAsStrMixin, FieldName, enum.Enum):
    """Field used in the NYTimes anomalies file"""
...
def read_nytimes_anomalies():
    ...
    # Denormalize so each row is a single date+location+metric anomaly
    df = _denormalize_nyt_anomalies(df)

    # Add LOCATION_ID column (must happen after denormalizing which can add additional rows for
    # subgeographies).
...
def _denormalize_nyt_anomalies(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Use end_date to create multiple rows to encompass the [date, end_date] range.
    df = df.assign(
        date=df.apply(
