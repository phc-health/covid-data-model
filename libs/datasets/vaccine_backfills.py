...
def estimate_initiated_from_state_ratio(ds_in: MultiRegionDataset) -> MultiRegionDataset:
    ...
    ts_bucketed_wide_dates_new = ds_in.timeseries_bucketed_wide_dates.copy()
    common_index = ts_bucketed_wide_dates_new.index.intersection(ts_counties_initiated_est.index)
    ts_bucketed_wide_dates_new.loc[common_index] = ts_counties_initiated_est.loc[common_index]
    ts_bucketed_wide_dates_new = ts_bucketed_wide_dates_new.append(
        ts_counties_initiated_est.loc[ts_counties_initiated_est.index.difference(common_index)]
    )
